...
def get_wikipedia_summary(subject):
    try:
        ...
        # Prompt user to choose from the disambiguation options
        print(f"Disambiguation error for '{subject}':")
        for idx, option in enumerate(e.options):
            print(f"{idx + 1}: {option}")
        choice = input(f"Please choose an option (1-{len(e.options)}): ")
        try:
            chosen_option = e.options[int(choice) - 1]
            return wikipedia.summary(chosen_option, sentences=5, auto_suggest=False, redirect=True)
...
def identify_question_type_and_subject(question):
    ...
    # extract the subject
    if question_type:
        # Extract the last noun chunk that isn't part of the question phrase
        subject_chunks = [chunk.text for chunk in doc.noun_chunks if chunk.root.head.text.lower() not in ["who", "what", "when", "where"]]
        if subject_chunks:
            subject = subject_chunks[-1].strip()
...
def log_sample(question, correct_answer, system_answer, correctness):
    with open("qa-samples.txt", "a", encoding="utf-8") as f:
        f.write(f"Question: {question}\n")
        f.write(f"Correct Answer: {correct_answer}\n")
        f.write(f"My System Answer: {system_answer} ({correctness})\n\n")
...
def main():
    ...
    # Clear the qa-samples.txt file if it exists
    with open("qa-samples.txt", "w", encoding="utf-8") as f:
        f.write("")
    ...
    print("The correct answer will be obtained from Wikipedia for logging purposes.")
    ...
    while True:
        ...
        if not question_type or not subject:
            ...
            system_answer = "I am sorry, I don't know the answer."
            correctness = "incorrect"
        else:
            system_answer = find_answer(question_type, subject, question)
            if system_answer:
                log_and_print(f"Answer => {system_answer}", 'info')
                if isinstance(system_answer, tuple):
                    correctness = "correct" if any(correct_answer.lower() in ans.lower() for ans in system_answer) else "incorrect"
                    system_answer = ' or '.join(system_answer)  # Combine tuple elements into a single string for logging
                else:
                    correctness = "correct" if correct_answer.lower() in system_answer.lower() else "incorrect"
            else:
                log_and_print("I am sorry, I don't know the answer.", 'info')
                system_answer = "I am sorry, I don't know the answer."
                correctness = "incorrect"

        log_sample(question, correct_answer, system_answer, correctness)
